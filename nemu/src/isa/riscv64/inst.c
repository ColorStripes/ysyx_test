...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();                                      //nemu/include/cpu/decode.h
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT(BITS(src1 << src2, 31, 0), 32));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = (((long int)src1 < (long int)src2) ? 1 : 0));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = ((src1 < src2) ? 1 : 0));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (long int)src1 >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = SEXT(((int)BITS(src1, 31, 0)) >> src2, 32));
  //INSTPAT("0001000 00010 00000 000 00000 11100 11", sre    , R,  );
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = src1 >> src2);
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = SEXT((BITS(src1, 31, 0) >> src2), 32));
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(dest) = SEXT(((int)BITS(src1, 31, 0) % (int)BITS(src2, 31, 0)), 32));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = SEXT((src1 % src2), 32));
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", remu   , R, R(dest) = SEXT(((long int)src1 % (long int)src2), 32));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  //INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, ExceptionReturn());
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (long int)src1 / (long int)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(dest) = SEXT((BITS(src1, 31, 0) / BITS(src2, 31, 0)), 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = SEXT(((long int)BITS(src1, 31, 0) / (long int)BITS(src2, 31, 0)), 32));
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = (((long int)src1 < (long int)src2) ? 1 : 0));
  INSTPAT("0000000 ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = ((src1 < src2) ? 1 : 0));
  
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (long int)src1 >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT(((int)BITS(src1, 31, 0)) >> src2, 32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> src2);
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT((BITS(src1, 31, 0) >> src2), 32));
  ...
  //B
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if((long int)src1 == (long int)src2) s->dnpc = s->pc + dest ); 
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((long int)src1 >= (long int)src2) s->dnpc = s->pc + dest ); 
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1 >= src2) s->dnpc = s->pc + dest ); 
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((long int)src1 < (long int)src2)  s->dnpc = s->pc + dest ); 
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1 < src2) s->dnpc = s->pc + dest );
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if((long int)src1 != (long int)src2) s->dnpc = s->pc + dest );
